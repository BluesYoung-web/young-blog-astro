...
<Alert class="my-4" type="warning">**只要是经过 Service Worker 的 fetch() 方法获取的资源，都会被标注为 from service worker**</Alert>
...
<Alert class="my-4" type="warning">**保证了同一页面的多个链接相同且类型相同的资源只会请求一次**</Alert>

<Alert class="my-4" type="warning">**在从 memory cache 获取缓存内容时，浏览器会忽视例如 max-age=0, no-cache 等头部配置，<span class="text-red-500">no-store 完全不会被缓存</span>**</Alert>
...
<Alert type="info">**优先级高于 Expires**</Alert>
...
<Alert type="info">**优先级高于 Last-Modified**</Alert>
