...
tableData:
  - name: none
    cond: 在通过构造函数创建 <code>Headers</code> 实例时激活
    limit: 无
  - name: request
    cond: 在通过构造函数初始化 <code>Request</code> 对象，且 <code>mode</code> 值为非 <code>no-cors</code> 时激活
    limit: 不允许修改禁止修改的头部
  - name: request-no-cors
    cond: 在通过构造函数初始化 <code>Request</code> 对象，且 <code>mode</code> 值为 <code>no-cors</code> 时激活
    limit: 不允许修改非简单头部
  - name: response
    cond: 在通过构造函数创建 <code>Headers</code> 实例时激活
    limit: 不允许修改禁止修改的响应头
  - name: immutable
    cond: 在通过 <code>error()</code> 或 <code>redirect()</code> 静态方法初始化 <code>Response</code> 对象时激活
    limit: 不允许修改任何头部
---
