...
<Alert title="修改属性的默认值" type="info">`Object.defineProperty(obj, propName, configObj)`</Alert>

<Alert title="configObj 中没有显示提供的属性都会默认为 false" type="warning" />

<Alert class="mt-4" title="configurable 定义为 false 之后不能再次定义为 true，再次修改任何非 writable 的属性都会导致错误" type="error" />
...
<Alert title="在不支持 Object.defineProperty() 的浏览器中没有办法修改 [[Configurable]] 或 [[Enumerable]]" type="warning" />
...
<Alert title="只定义获取函数意味着属性是只读的" type="warning">尝试修改属性会被忽略。在严格模式下，尝试写入只定义了获取函数的属性会抛出错误</Alert>
...
<Alert title="只定义设置函数会导致无法读取" type="error">非严格模式下返回 `undeifned`，严格模式下抛出错误</Alert>
