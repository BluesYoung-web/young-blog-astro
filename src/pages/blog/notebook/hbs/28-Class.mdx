...
<Alert title="只能在构造函数、实例方法、静态方法中使用" type="info" />

<Alert class="mt-4" title="后代构造函数第一行必须执行 super()，否则不能使用 this" type="warning" />

<Alert class="mt-4" title="不能单独引用 super 关键字" type="warning">要么用它调用构造函数，要么用它引用静态方法</Alert>
