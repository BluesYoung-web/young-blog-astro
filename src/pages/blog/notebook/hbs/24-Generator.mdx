...
<Alert title="箭头函数不能用来定义生成器函数" type="warning" />
...
<Alert title="生成器函数只会在初次调用 next() 方法后开始执行" type="info" />
...
<Alert title="第一次调用 next() 传入的值不会被使用" type="info" />
...
<Alert title="错误处理会跳过对应的 yield" type="info" />

<Alert class="mt-5" title="如果生成器对象还没有开始执行，那么调用 throw() 抛出的错误不会在函数内部被捕获，因为这相当于在函数块外部抛出了错误" type="warning" />
